    ...
    course.map((val,i)=>{
      return(
        <tr className={`${(i%2)!==0?'bg-gray-200':''}`}>
          <td className='px-3 py-3 border-2  border-gray-500 '>{i+1}</td>
          <td className=' py-3 px-5 text-center border-2 border-gray-500 '>
            <Link to={`/Course/${val.id}`}>
            {val?.title}
            </Link>
            </td>
          <td className='px-3 text-center py-3 border-2  border-gray-500 '>{val?.duration}</td>
          <td className='px-3 py-3 text-center border-2  border-gray-500 '>
            <ol type="1">
          ...
          <td    className='px-3 py-3 text-center border-2  border-gray-600 '>{val?.category.name}</td>
